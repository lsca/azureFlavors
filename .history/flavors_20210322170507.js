                ...
                try {
                    ...
                    response.Items.map((item) => {
                        if (item.skuName.indexOf('Spot') > -1) {
                            console.log(item.armSkuName);
                            fs.appendFile("test.sql", `INSERT INTO usavingsFlavorPrices value (spot) values (${item.ratailPrice}) where name=${item.armSkuName};`, function (err) {
                                if (err) throw err;
                                console.log('Saved!');
                            });
                        }
