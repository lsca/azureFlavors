...
rq.on('resolved', res => {
    ...
    response.Items.map((item) => {
        if (item.serviceName == "Virtual Machines") {
            console.log(item);
            // fs.appendFile("test.sql", `INSERT INTO usavingsflavorsprice (name, vCpu, memory, osType, price, diskType, currency, location, reservedCostOneYear, type) values ('${item.armSkuName}',0,0,'${item.productName}', '${item.retailPrice}', 'SSD','${item.currencyCode}', '${item.armRegionName}', 0, azure);`, function (err) {
            //     if (err) throw err;
            //     console.log('Saved!');
            // });
        }
