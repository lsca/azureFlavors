...
rq.on('resolved', res => {
    ...
    if (res.status == '1') {
      const pod = res.msg;
      const insert = `INSERT INTO podPerformance (id, name, namespace, clusterId, cpuUsage, memoryUsage, cpuPrice, memoryPrice, resourceParent, parentName, cpuMonitor, memMonitor, userId)
                                    values ('${pod.uuid}', '${pod.podName}', '${pod.namespace}', ${pod.clusterId}, ${pod.cpuConsume}, ${pod.memConsume}, ${pod.cpuPrice}, ${pod.memoryPrice}, '${pod.parent}', '${pod.parentName}',${pod.cpuMonitor},${pod.memMonitor},${pod.userId*1})`;
      dao.billingPersistence([insert]);
    } else {
      console.log(res);
    }
  }).on('rejected', err => {
